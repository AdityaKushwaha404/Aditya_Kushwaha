...
const Clients = () => {
  useEffect(() => {
    ...
    gsap.fromTo(
      ".cards-window",
      { clipPath: "inset(0 0 100% 0 round 24px)", opacity: 0.5 },
      {
    ...
    cards.forEach((card, idx) => {
      ...
      gsap.fromTo(
        card,
        { x: fromX, y: 16, opacity: 0, filter: "blur(2px)" },
        {
          x: 0,
          y: 0,
          opacity: 1,
    ...
    // Hover/focus micro-interactions + accent-bar expand
    cards.forEach((card) => {
      ...
      card.addEventListener("focus", () => tlHover.play());
      card.addEventListener("blur", () => tlHover.reverse());
    });

    // Sequential pop-in for skill chips
    ...
    // Ambient floating loop (disabled on coarse pointers)
    if (!coarsePointer) {
    ...
    // Mousemove background parallax + spotlight (disabled on coarse pointers)
          ...
          <div className="mt-8">
            ...
            <div className="skills-row mt-3 flex flex-wrap gap-2">
              {softSkills.map((s) => (
                <span
                  key={s}
                  className="skill-chip px-3 py-1 text-xs md:text-sm rounded-full bg-white/6 border border-white/12 text-white/85 backdrop-blur-xl hover:bg-white/10 hover:shadow-[0_4px_16px_rgba(124,58,237,0.25)] transition-all"
                >
                  {s}
                </span>
              ))}
